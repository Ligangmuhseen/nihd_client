...
const PrivacyTable = () => {
  const [privacy, setPrivacy] = useState([]); // Initialize as an empty array
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    // Fetch data from your API
    axios.get(`${API_BASE_URL}/privacy/upload/`) // Replace with your API URL
      .then((response) => {
                ...
                <tbody>
                  {privacy.map((priv) => (
                    <tr key={priv.id}>
                      ...
                      <td>
                        <p className="fw-normal">{priv.privacies}</p>
                      </td>
                      
                      <td className="text-center py-0 align-middle">
                        <div className="btn-group btn-group-sm">
                          <Link
                            to={`privacyform/${priv.id}`}>
                              <button
